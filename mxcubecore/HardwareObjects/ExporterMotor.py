...
class ExporterMotor(AbstractMotor):
    ...
    def init(self): 
        ...
        self.chan_position = self.addChannel({"type": "exporter", 
        ...
        self.chan_state = self.addChannel({"type":"exporter",
                                           "name":"state" },
                                           "State")
        self.chan_all_motor_states = self.addChannel({"type":"exporter",
                                                      "name":"motor_states"}, "MotorStates")
        self.chan_all_motor_states.connectSignal("update", self.all_motor_states_changed)

        self.cmd_abort = self.addCommand({"type":"exporter",
                                          "name":"abort" },
                                         "abort")
        self.cmd_get_dynamic_limits = self.addCommand({"type": "exporter",
                                                       "name": "get%sDynamicLimits" % self.motor_name},
                                                       "getMotorDynamicLimits")
        self.cmd_get_limits = self.addCommand({"type": "exporter",
                                               "name": "get_limits"},
                                              "getMotorLimits")
        self.cmd_get_max_speed = self.addCommand({"type": "exporter",
                                                  "name": "get_max_speed"},
                                                 "getMotorMaxSpeed")
        self.cmd_home = self.addCommand({"type":"exporter",
                                         "name":"homing"},
                                        "startHomingMotor")
