...
class MaxLabISPybClient2(HardwareObject):
    """
    Web-service client for ISPyB.
    """

    def __init__(self, name):
#        define the authentication server type (authServerType, ispyb, ldap) and login Type (loginType, proposal or user) in HO.xml
...
#                      'Protein': {}})
    ...
    def find_detector(self, type, manufacturer,
        ...
        if self.__collection:
            try:
                logging.getLogger().debug("detector: manufacturer:%s, model:%s, mode:%s" % (manufacturer, model, mode))
                res= self.__collection.service.\
                       findDetectorByParam("", manufacturer, model, mode)
                logging.getLogger().debug("detector: %s" % res)
                return res
            except WebFault:
                logging.getLogger("ispyb_client").\
