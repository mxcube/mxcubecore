...
class XSDataInputControlDozor(XSDataInput):
    def __init__(
        ...
        if directory is None:
            ...
            strMessage = (
                "ERROR! XSDataControlImageDozor constructor argument 'directory' is not XSDataString but %s"
    ...
    def directory(self, directory):
        if directory is None:
            ...
            self._directory = doISPyBUpload
        else:
            strMessage = (
                "ERROR! XSDataInputControlDozor.directory argument is not XSDataString but %s"
    ...
    def exportChildren(self, outfile, level, name_="XSDataInputControlDozor"):
        ...
        if self._first_image_number is not None:
            self.first_image_number.export(outfile, level, name_="startNo")
        else:
            warnEmptyAttribute("first_image_number", "XSDataInteger")
        if self._last_image_number is not None:
            self.last_image_number.export(outfile, level, name_="endNo")
        else:
        ...
        if self._directory is not None:
            showIndent(outfile, level)
            outfile.write(unicode('<directory>\n'))
            self.directory.export(outfile, level+1, name_="path")
            showIndent(outfile, level)
            outfile.write(unicode('</directory>\n'))
