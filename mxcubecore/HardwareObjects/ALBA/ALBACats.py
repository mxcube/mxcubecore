...
class ALBACats(Cats90):
	...
	"""
        ...
        if self.read_super_phase().upper() == "SAMPLE":
            logging.getLogger("user_level_log").error("Supervisor is already in sample view phase")
            return True
        ...
        while True:
            ...
            if (time.time() - t0) > TIMEOUT:
                logging.getLogger("user_level_log").error("Supervisor timeout waiting for ON state. Returning") 
                return False
    ...
    def save_detdist_position(self):
        ...
        logging.getLogger("user_level_log").error("Saving current det.distance (%s)" % self.detdist_saved)

    def restore_detdist_position(self):
        if abs(self.detdist_saved - self.detdist_position_channel.getValue()) >= 0.1:
            logging.getLogger("user_level_log").error("Restoring det.distance to %s" % self.detdist_saved)
    ...
    def load(self, sample=None, wait=False, wash=False):
        ...
        return self._executeTask(SampleChangerState.Loading,wait,self._doLoad,sample,None,use_ht)
    ...
    def _doLoad(self, sample=None, shifts=None, use_ht=False):
        ...
        if self.cats_sample_on_diffr() == -1:
            self._updateState() # remove transient states like Loading. Reflect hardware state
            raise Exception("Conflicting info between diffractometer and on-magnet detection. Consider 'Clear'")
            return
        ...
        if use_ht:  # loading HT sample
            ...
            if loaded_ht == 1:  # has ht loaded
                cmd_ok = self._executeServerTask(self._cmdChainedLoadHT, argin, waitsafe=True)
            else:
        ...
        #
            ...
            if tool != current_tool:
                logging.getLogger("HWR").warning("  ==========CATS=== changing tool from %s to %s" % (current_tool, tool))
                changing_tool = True
            else:
                changing_tool = False
            ...
            if tool == 2:
                read_barcode = self.read_datamatrix and self._cmdChainedLoadBarcode is not None
            else:
                if self.read_datamatrix: 
                    logging.getLogger("HWR").warning("  ==========CATS=== reading barcode only possible with spine pucks")
                read_barcode = False

            if loaded_ht == -1:  # has a loaded but it is not an HT

                if changing_tool:
                     raise Exception("This operation requires a tool change. You should unload sample first")

                if read_barcode:
                    logging.getLogger("HWR").warning("  ==========CATS=== chained load sample (barcode), sending to cats:  %s" % argin)
                    cmd_ok = self._executeServerTask(self._cmdChainedLoadBarcode, argin, waitsafe=True)
                else:
                    logging.getLogger("HWR").warning("  ==========CATS=== chained load sample, sending to cats:  %s" % argin)
                    cmd_ok = self._executeServerTask(self._cmdChainedLoad, argin, waitsafe=True)
            elif loaded_ht == 0:
                if read_barcode:
                    logging.getLogger("HWR").warning("  ==========CATS=== load sample (barcode), sending to cats:  %s" % argin)
                    cmd_ok = self._executeServerTask(self._cmdLoadBarcode, argin, waitsafe=True)
                else:
                    logging.getLogger("HWR").warning("  ==========CATS=== load sample, sending to cats:  %s" % argin)
                    cmd_ok = self._executeServerTask(self._cmdLoad, argin, waitsafe=True)
