...
class ISPyBClientMockup(HardwareObject):
    ...
    def init(self):
        ...
        self.__test_proposal = {
            ...
            "Session": {
                    ...
                    "timeStamp": datetime.datetime.strftime(datetime.datetime(2013, 6, 11, 9, 40, 36),
                                                            "%Y-%m-%d %H:%M:%S"),
    ...
    def login(self, loginID, psd, ldap_connection=None, create_session=True):
        ...
        ok, msg = self.ldap_login(loginID, psd, ldap_connection)
        logging.getLogger("HWR").debug(
            " searching for user %s / psd: %s. It is %s" % (loginID, psd, ok)
        )
        ...
        """ The information from self.__test_proposal is an example of the information format, the correct information 
        should be retrieved using "get_proposal" from the LDAP server """
    ...
    def ldap_login(self, login_name, psd, ldap_connection):
        if self.ldap_connection is None:
            self.ldap_connection = self.ldapConnection

        ok, msg = self.ldap_connection.login(login_name, psd, fields=["uid"])
        return ok, msg
    ...
    def get_proposal(self, proposal_code, proposal_number):
        ...
        :rtype: dict
        """
        prop = self.__test_proposal
        values = self.ldap_connection.get_field_values()
        user_id = (values['uid'][0]).decode()
        if user_id == proposal_code + proposal_number:
            prop["Proposal"]["code"] = proposal_code
            prop["Proposal"]["number"] = proposal_number
