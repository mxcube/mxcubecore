...
class Xtal(Sample):
    ...
    def __init__(self, drop, index):
        super().__init__(drop, self._get_xtal_address(drop, index), False)
        self._drop = drop
        self._index = index
    ...
    def get_name(self):
        return f"{self.get_cell().get_row_chr()}{self.get_cell().get_index() + 1}:{self._drop.get_index() + 1}"
...
class Drop(Container):
    ...
    def __init__(self, cell, drops_num):
        super().__init__(
            self.__TYPE__, cell, self._get_drop_address(cell, drops_num), False
        )
        self._cell = cell
...
class PlateManipulator(SampleChanger):
    ...
    def init(self):
        ...
        self.plate_location_changed(self.diffr.plate_location.get_value())

        super().init()

    def change_plate_barcode(self, barcode: str) -> bool:
        ...
        if self._load_data(barcode):
            ...
            return True
        raise Exception("Unknown barcode")
    ...
    def plate_location_changed(self, plate_location: tuple | None = None):
    ...
    def _load_sample(self, sample_location=None, pos_x=None, pos_y=None):
        ...
        try:
            ...
            if hasattr(self.diffr, "move_to_drop"):
                ...
                col += 1
                _addr = f"{chr(65 + row)}{col}"
                cell = self.get_component_by_address(_addr)
                drop = cell.get_component_by_address(f"{_addr}:{drop}")
    ...
    def _load_data(self, plate_barcode):
        ...
        if processing_plan is None:
            msg = f"No information for plate with barcode {plate_barcode} in CRIMS"
            logging.getLogger("user_level_log").error(msg)
        else:
            msg = "Information for plate with barcode {plate_barcode} in CRIMS"
    ...
    def get_sample(self, plate_location):
        """Convert the plate location to sample name
        Args:
            plate_location: The location
        """
        row = int(plate_location[0])
        col = int(plate_location[1])
        y_pos = float(plate_location[3])
        drop_index = abs(y_pos * self.num_drops) + 1
        drop_index = min(drop_index, self.num_drops)

        _addr = f"{chr(65 + row)}{(col + 1)}"
        cell = self.get_component_by_address(_addr)
        if cell:
            drop = cell.get_component_by_address(f"{_addr}:{drop_index}")
