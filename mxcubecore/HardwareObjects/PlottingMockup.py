...
def plot_emitter(new_plot, plot_data, plot_end):
  ...
  info = {"scan_nb": scan_nb, "title": "Test data %d" % scan_nb, "labels": ["angle", "diode value"]} 
...
class PlottingMockup(HardwareObject):
  ...
  def init(self, *args):
     self.__plotter = gevent.spawn(plot_emitter, self.__on_scan_new, self.__on_scan_data, self.__on_scan_end)
