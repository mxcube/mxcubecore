...
class BL19U1BeamlineActions(HardwareObject):
    def __init__(self, *args):
        HardwareObject.__init__(self, *args)

    def init(self):
        # self.cleardetector = ControllerCommand("Clear Eiger2X", None, "Clear Detector", klass=ClearDetectorAction)
        self.cleardetector = ControllerCommand("Clear Detector", None, "Clear Detector", klass=ClearDetectorAction)
    ...
    def get_commands(self):
        return [self.cleardetector, self.centrebeam, self.quick_realign, self.anneal, self.combotest]
