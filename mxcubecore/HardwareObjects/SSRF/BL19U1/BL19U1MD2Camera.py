...
class BL19U1MD2Camera(Device):
    ...
    def __init__(self, name):
        ...
        self.bzoom_device = HWR.beamline.bzoom
    ...
    def poll(self):
        logging.getLogger("HWR").info("going to poll images, pollInterval: %d",self.pollInterval)
        # self.image_attr = self.add_channel(
        #     {"type": "exporter", "name": "image"}, "ImageJPG"
        # )
        count = 1
        while not self.stopper:
            ...
            try:
                ...
                img, width, height = self.bzoom_device.get_jpg_image()
                # print(type(img))
                # img = self.bzoom_device.image_attr.get_value()
                # print("got the image", datetime.datetime.now().strftime("%H:%M:%S.%f"))
                ...
                self.emit("imageReceived", imgStr, width, height)
                count = count + 1
