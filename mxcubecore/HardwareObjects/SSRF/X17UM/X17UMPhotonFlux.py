...
class X17UMPhotonFlux(AbstractFlux):
    ...
    def __init__(self, name):
        ...
        self._aperture = None
        self.beam_flux_chan = None

    def init(self):
        """Initialisation"""
        AbstractFlux.init(self)
        # controller = self.getObjectByRole("controller")
        # ultimately it should be HWR.beamline.diffractometer.aperture
        self.beam_flux_chan = self.get_channel_object("beamFlux")
        self.beam_flux_chan.connect_signal("update", self.update_value)
        ...
        self._counter = self.beam_flux_chan.get_value()

        # HWR.beamline.safety_shutter.connect("shutterStateChanged", self.update_value)
