...
class SSRFEnergyScan(AbstractEnergyScan, HardwareObject):

    # define some constants taken from PX1-SOLEIL
    integration_time = 1
    default_steps = 20
    round_cutoff = 4
    roi_width = 0.30
    ...
    def init(self):
        ...
        # self.mono_dp = self.getProperty("mono_dev")
        # self.ble_dp = self.getProperty("ble_dev")
    ...
    def open_fast_shutter(self):
        HWR.beamline.fast_shutter._set_value(True)

    def close_fast_shutter(self):
        HWR.beamline.fast_shutter._set_value(False)

    def fluodet_prepare(self):
        self.fluodet_hwo.set_presets(ctime=1, erange=1, fname=None)
    ...
    def move_energy(self, energy):
        ...
        self.wait_device(HWR.beamline.energy)
    ...
    def get_egy_values_to_scan(self):
        ...
        points = numpy.arange(0.0, 1.0 + 1.0 / (nb_steps), 1.0 / (nb_steps))  # TODO get correct value ranges
        points = points.astype(float)
        points *= self.scan_range   # TODO get the correct range
        points -= self.before_edge  # TODO get correct value
        points += 12  # self.e_edge TODO get correct edge energy value from edge name, now retrieving "K" or "L"
        # points = numpy.array(map(self.round_egy, points)
        points = numpy.round(points, self.round_cutoff)

        self.log.info(
            "Preparing scan around edge for %s/%s :"
    ...
    def escan_prepare(self):
        ...
        HWR.beamline.diffractometer.set_phase("DataCollection", wait=True) # to avoid collision with the Fluo detector nozzle

        # self.set_mca_roi() TODO set this properly when we have the config_file from ESRF with all information of roi per element
        self.fluodet_prepare()

        self.points = self.get_egy_values_to_scan()
        self.log.info("Energy points : %s" % str(self.points))
        ...
        self.scan_info["fluorescenceDetector"] = " Vortex Mockup detector"

    def acquire_point(self, en):
        self.open_fast_shutter()
        self.fluodet_hwo.start_acq()
        # TODO check if acquisition time has to be defined
        #  by software sleep of via device config (give a parameter to start_acq)
        self.fluodet_hwo.stop_acq()
        # self.fluodet_hwo.wait_ready() TODO re-enable this line once the detector is operational
        self.close_fast_shutter()
        roi_counts = self.fluodet_hwo.get_roi_count()
        intensity = 1 #self.norm_diode_dev.intensity

        point = float(roi_counts / intensity)  # TODO normalize by beam flux ?
        return point
    ...
    def startEnergyScan(
        ...
        self.integration_time = exptime

        try:
            self.escan_prepare()
            self.scanCommandStarted()

            # activate line below for simulation
            ...
            self.emit("scanStatusChanged", ("error starting energy scan",))
            return False
