"""
  File:  BL19U1Collect derived from BIOMAXCollect.py
...
from HardwareRepository.HardwareObjects.SSRF.BL19U1.BL19U1DetCover import DetCover
from HardwareRepository.HardwareObjects.SSRF.BL19U1 import Constants as cts
...
class BL19U1Collect(AbstractCollect, HardwareObject):
    ...
    def init(self):
        ...
        self.images_per_file = self.getProperty("images_per_file", 1)
        ...
        try:
            ...
            logging.getLogger("HWR").error(
                "[HWR] *** Detector X pixel size not available, set to 1.72e-4"
            )
            pix_x = 1.72e-4
        try:
            ...
            logging.getLogger("HWR").error(
                "[HWR] *** Detector Y pixel size not available, set to 1.72e-4"
            )
            pix_y = 1.72e-4
    ...
    def do_collect(self, owner):
        ...
        try:
            ...
            expo_time = self.current_dc_parameters["oscillation_sequence"][0]["exposure_time"]
            if expo_time>=0.5 and expo_time <=0.53:
                self.current_dc_parameters["oscillation_sequence"][0]["exposure_time"] = 0.49
    ...
    def prepare_acquisition(self):
        ...
        if "wavelength" in self.current_dc_parameters:
            ...
            try:
                ...
                HWR.beamline.detector.set_wavelength(self.current_dc_parameters["wavelength"])
            except Exception as ex:
            ...
            try:
                ...
                wl = 12.398 / self.current_dc_parameters["energy"]
                HWR.beamline.detector.set_wavelength(wl)
    ...
    def data_collection_hook(self):
        ...
        try:
            ...
            try:
                ...
                #a=num_images/100+1
                a = num_images + 1
                # while self.last_image_saved() < a and time.time() < timeout_time:
                #     time.sleep(0.5)
                #
    ...
    def wait_for_file_copied(self, full_file_path):
        ...
        size1 = -1
        with gevent.Timeout(
            800, Exception("Timeout waiting for the data to be copied available.")
        ):
...
#            HWR.beamline.diffractometer.beam_position_vertical.get_value(),
 #       )
    ...
    def set_energy(self, value):
        ...
        HWR.beamline.detector.set_energy_threshold(value)  # ev
    ...
    def prepare_input_files(self):
        ...
        i = 1
        logging.getLogger("user_level_log").info(
            "Creating XDS (NCPSS-17B) processing input file directories"
        )
    ...
    #         return nimages
    def get_det_filenumber(self):
        number = HWR.beamline.detector.getfilenumber()
        return number

        """
            Description: returns the number of images saved once detector was armed
        """
        number = HWR.beamline.detector.get_file_list()
        return number + 1
    ...
    def prepare_detector(self):
        ...
        config["photon_energy"] = self.current_dc_parameters["energy"]    # must be in keV in detector
        ...
        file_parameters["filename"] = "%s_master.cbf" % name_pattern
        ...
        #    image_file_template,
        # )
        logging.getLogger("HWR").info(config)
        number = self.current_dc_parameters["oscillation_sequence"][0]["start_image_number"]
        HWR.beamline.detector.setFileNumber(number)

        HWR.beamline.energy.set_det_energy()
    ...
    def prepare_for_new_sample(self, manual_mode=True):
        ...
        if HWR.beamline.detector.distance.get_value() < 250:
            HWR.beamline.detector.distance.set_value(250)
    ...
    def store_datacollection_uuid_datacatalog(self):
        ...
        msg["beamline"] = "BL19U1"
        msg["event"] = "BL19U1-experiment-began"
