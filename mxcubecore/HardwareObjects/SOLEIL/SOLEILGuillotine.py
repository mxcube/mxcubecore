...
class SOLEILGuillotine(BaseHardwareObjects.Device):
    ...
    def getShutterState(self):
        ...
        if not self.checkDistance():
            return "disabled"
        return SOLEILGuillotine.shutterState.get(
    ...
    def modeCollect(self):
        ...
        logging.info(
            "SOLEILGuillotine - setting gonio currentPosition is   %s" % currentDistance
        )
        if self.isInsert():
            if not self.checkDistance():
                # self.detector.move(180)
                self.detector.move(self._d_security)
                time.sleep(2.0)
                ...
                self.detector.move(currentDistance)
                time.sleep(2.0)
                while self.detector.motorIsMoving():
                    time.sleep(0.5)
            else:
