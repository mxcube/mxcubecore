...
class PX1EnergyScan(AbstractEnergyScan,Equipment):
    ...
    def set_mca_roi(self):
        ...
        self.roi_start_chan = int(egy_start / (B*1.e3)) 
    ...
    def escan_prepare(self):
        ...
        self.log.info("   edge for %s/%s is %s" % (self.scan_info['element'], self.scan_info['edgeEnergy'],self.e_edge))
        ...
        self.scan_info["startEnergy"] = 0. # updated at the end of the scan
        self.scan_info["endEnergy"] = 0. # updated at the end of the scan

        try:
            size_hor, size_ver = self.beam_info_hwo.get_beam_size()
            size_hor *= 1000
            size_ver *= 1000
        except:
            size_hor = None
    ...
    def startEnergyScan(self,element,edge,directory,prefix, \
        ...
        if not os.path.isdir(directory):
            ...
            try:
                ...
                log.error("EnergyScan: error creating directory %s (%s)" % (directory,str(diag)))
        ...
        try:
            ...
            import traceback
            logging.getLogger("HWR").error('EnergyScan: problem starting energy scan. %s' % traceback.format_exc())
    ...
    def doChooch(self, elt, edge, scan_directory, archive_directory, 
                   prefix, run_number=None):
        ...
        # run chooch command
        self.log.info("EnergyScan. running chooch %s %s %s %s" % (self.chooch_cmd, scan_file_efs_filename, elt, edge))
        self.log.info("   on success efs file should be saved : %s" % scan_file_efs_filename)
        try:
             p = subprocess.Popen([self.chooch_cmd,
                                  scan_file_raw_filename,
                                  elt,
                                  edge,
                                  scan_file_efs_filename],
             ...
             for line in chooch_result_lines:
                 ...
                 if line == 'chooch_results':
                     result_line = self.clean_chooch_line(chooch_result_lines[next_lineno])
                     found = 1
                     break
             ...
             if found:
                 ...
                 self.store_energy_scan()
                 logging.getLogger("HWR").error("Energy scan: Chooch cannot parse results")
                 return 
        ...
        if math.fabs(self.thEdge - ip) > self.thEdgeThreshold:
            if (self.thEdge - ip) > 0.02: 
                ...
                side = "above"
            logging.info('PX1EnergyScan. Theoretical edge too different from the one just determined. thEdgeThreshold = %.2f' % self.thEdgeThreshold)
            pk = 0
            ip = 0
            rm = self.thEdge + 0.03
   
            logging.getLogger("HWR").warning("EnergyScan: calculated peak is %s theoretical value more than 20eV" % side)
        ...
        logging.getLogger("HWR").warning("  efs file has been archived at %s" % archive_file_efs_filename)
        ...
        logging.getLogger("HWR").warning("  generating graph data from %s" % str(chooch_graph_data))
        ...
        try:
          logging.getLogger("HWR").info("Rendering energy scan and Chooch graphs to PNG file : %s", archive_file_png_filename)
