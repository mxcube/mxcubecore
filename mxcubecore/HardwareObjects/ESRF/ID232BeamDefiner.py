...
class ID232BeamDefiner(HardwareObject):
    ...
    def init(self):
        ...
        for lens_cfg in cfg:
              ...
              self.tfCfgByName[name] = { "tf1": ["IN" if x else "OUT" for x in map(int, tf1)], "tf2": ["IN" if x else "OUT" for x in map(int, tf2)] }
    ...
    def getCurrentPositionName(self, *args):
        ...
        for name in self.posNames:
            ...
            for i, tf_pos in enumerate(zip(tf1_state, tf2_state)):
                if tf_pos[0] in (tf1_cfg[i], "---") and tf_pos[1] in (tf2_cfg[i], "---"):
                    continue
                else:
                    break
            else:
