...
class MicrodiffKappaMotor(ExporterMotor):
    ...
    def init(self):
        ...
        if self.actuator_name == "Kappa":
            MicrodiffKappaMotor.conf["KappaTrans"] = self.stringToList(self.get_property("kappaTrans"))
            MicrodiffKappaMotor.conf["KappaTransD"] = self.stringToList(
                self.get_property("kappaTransD")
            )
        elif self.actuator_name == "Phi":
            MicrodiffKappaMotor.conf["PhiTrans"] = self.stringToList(self.get_property("phiTrans"))
            MicrodiffKappaMotor.conf["PhiTransD"] = self.stringToList(self.get_property("phiTransD"))
