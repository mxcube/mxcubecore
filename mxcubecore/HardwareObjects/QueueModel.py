...
class QueueModel(HardwareObject):
    ...
    def select_model(self, name):
        ...
        HWR.beamline.config.queue_manager.clear()
    ...
    def view_created(self, view_item, task_model):
        ...
        if isinstance(task_model, queue_model_objects.Sample):
            HWR.beamline.config.queue_manager.enqueue(qe)
    ...
    def get_all_queue_entries(self):
        ...
        for qe in HWR.beamline.config.queue_manager._queue_entry_list:
            get_nodes_list(qe)
    ...
    def save_queue(self, filename=None):
        ...
        queue_entry_list = HWR.beamline.config.queue_manager.get_queue_entry_list()
    ...
    def load_queue_from_json_list(self, queue_list, snapshot):
        ...
        sample_dict = {}
        for item in HWR.beamline.config.queue_manager.get_queue_entry_list():
            if isinstance(item, queue_entry.SampleQueueEntry):
