...
class MAXLABEnergyScan(Equipment):
    def init(self):
        ...
        if self.energy2WavelengthChannel is None:
            logging.getLogger("HWR").error(
                "EnergyScan: error initializing energy-wavelength constant (missing channel)"
            )
    ...
    def sConnected(self):
        if (
            self.energy2WavelengthChannel is not None
            and self.energy2WavelengthConstant is None
        ):
            try:
            ...
            try:
                ...
                try:
                    ...
                    logging.getLogger("HWR").debug(
                        "EnergyScan: default wavelength is %f" % self.defaultWavelength
                    )
    ...
    def doChooch(self, scanObject, elt, edge, scanArchiveFilePrefix, scanFilePrefix):
        ...
        logging.getLogger("HWR").exception(
            "running chooch in mxcube (%s)" % scanArchiveFilePrefix
        )
        ...
        try:
            ...
            if scanObject is None:
                raw_data_file = os.path.join(
                    os.path.dirname(scanFilePrefix), "data.raw"
                )
                try:
