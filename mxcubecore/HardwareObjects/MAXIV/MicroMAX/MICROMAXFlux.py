...
class MICROMAXFlux(AbstractFlux):
    ...
    def _calc_flux(self, energy, dist, current):
        ...
        al_transmission = self.transmit(
            self.al_thickness, energy / 1000.0, self.al_coeff
        )
        print(
            "Air transmission is {} al_transmission is {}".format(
                air_transmission, al_transmission
            )
        )
    ...
    def check_beam_opt(self):
        ...
        for i in range(1, 5):
            ...
            total += current
        flux = total * (
            -0.534515 * energy_ev * energy_ev * energy_ev * energy_ev
            - 43197.6 * energy_ev * energy_ev * energy_ev
            + 5.13449e09 * energy_ev * energy_ev
            - 4.39169e13 * energy_ev
            + 1.14591e17
        )
        final_msg = (
            "The currents on b312a-o06-ctl-aemhv-01 are {} sum value is {}".format(
                msg, total
            )
        )
        print(final_msg)
