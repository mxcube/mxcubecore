...
class BIOMAXFlux(AbstractFlux):
    ...
    def init(self):
        ...
        self.air_length = self.detector_distance_hwobj.get_value() * 1000 + \
    ...
    def si_diode_flux(self, current, transmission, absorption, energy):
        ...
        if abs(self.e * energy * sipha_absorp * transmission) == 0:
            ...
            flux = current * epsilon / \
                (self.e * energy * sipha_absorp * transmission)
            return(float(flux))
    ...
    def _get_instant_flux(self):
        ...
        air_length = self.detector_distance_hwobj.get_value() * 1000 + \
            self.airsample_length
    ...
    def update_flux_density(self):
        try:
            ...
            self.flux_density = float(self.current_flux)  / float(transmission) / beamx  / beamy / 10000
    ...
    def estimate_flux(self):
        ...
        if self.flux_aem_dev is None:
            msg = "Error: AEM for esitmating flux is not defined or cannot connect to it"
            self.logger.error(msg)
            raise Exception(msg)
        else:
            current = self.flux_aem_dev.S
        flux = current * ( 2.70541e-06 * energy_ev*energy_ev -0.00978719 * energy_ev - 30.1795) * 7.96e14
