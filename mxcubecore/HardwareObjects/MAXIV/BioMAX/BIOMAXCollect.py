...
class BIOMAXCollect(AbstractCollect, HardwareObject):
    ...
    def init(self):
        ...
        self.set_beamline_configuration(
                ...
                detector_manufacturer=self.detector_hwobj.get_property(
                    "manufacturer"
                ),
    ...
    def do_collect(self, owner):
        ...
        try:
            ...
            try:
                ...
                if not os.path.exists(archive_directory):
                    try:
                        ...
                        logging.getLogger("HWR").exception("Collection: Error creating archive directory")

                os.chmod(archive_directory, 0o777)
                for file in snapshots_files:
                    os.chmod(file, 0o777)
            except Exception as ex:
                log.error("[COLLECT] Archive directory preparation failed. Data collection continues. Error was: %s" %str(ex))
            ...
            if self.char: # correct the omega values in the master file for characterization
                ...
                self.correct_omega_in_master_file(filename = master_filename,overlap = overlap)
    ...
    def generate_and_copy_thumbnails(self, data_path, frame_number):
        ...
        cmd = "ssh b-biomax-eiger-dc-1 python %s -input %s -binfactor %d -output %s -start %d -nimages %d &" \
            % (script, input_file, binfactor, jpeg_thumbnail_full_path, first_image, nimages)
    ...
    def _store_image_in_lims(self, frame_number, motor_position_id=None):
        ...
        if self.lims_client_hwobj:
            ...
            if archive_directory:
                ...
                lims_image["fileLocation"] = self.current_dc_parameters["fileinfo"]["directory"]
            if motor_position_id:
                lims_image["motorPositionId"] = motor_position_id
            logging.getLogger("HWR").info("LIMS IMAGE: %s, %s, %s, %s" %(jpeg_filename, thumb_filename, jpeg_full_path, jpeg_thumbnail_full_path))
            try:
    ...
    def move_detector(self, value):
        ...
        logging.getLogger("HWR").info("...................value %s, detector movement start..... %s" % (value, self.dtox_hwobj.get_value()))
        ...
        logging.getLogger("HWR").info("....................value %s detector movement finished.....%s" % (value, self.dtox_hwobj.get_value()))

        current_pos = self.dtox_hwobj.get_value()
        if abs(current_pos - value) > 0.05:
            logging.getLogger("user_level_log").exception("Detector didn't go to the set position")
            self.stop_collect()
    ...
    def prepare_dozor_input(self, oscillation_parameters, path):
        ...
        try:
            ...
            logging.getLogger("HWR").error("[COLLECT] Configuing Dozor input Error: %s" % ex)
            raise RuntimeError("[COLLECT] Error while trying to get parameters for Dozor.")
    ...
    def start_spot_finder_dozor(self):
        ...
        dozor_dat_path = self.prepare_dozor_input(oscillation_parameters, dozor_path) or None
    ...
    def get_instant_flux(self, keep_position=False):
        ...
        try:
            ...
            logging.getLogger("HWR").error("[COLLECT] Cannot get the current flux value")
            flux = -1
    ...
    def prepare_set_energy(self):
        ...
        try:
            if float(self.get_machine_current()) < 10.0:
                ...
                logging.getLogger("user_level_log").warning("[COLLECT] Very low ring current")
            ...
            if self.ha.StatusClosed:
                ...
                logging.getLogger("user_level_log").warning("[COLLECT] Heat Absorber Closed")

            self.aem = PyTango.DeviceProxy("b311a/xbpm/02")
            if self.aem.S < 1e-7:
                checkbeam = False
                logging.getLogger("HWR").warning("[COLLECT] No beam measured")
                logging.getLogger("user_level_log").warning("[COLLECT] No beam measured")

        except Exception as ex:
            logging.getLogger("HWR").error("[COLLECT] Cannot prepare_set_energy,checkbeam: %s, error was: %s" % (checkbeam ,ex))
    ...
    def store_datacollection_uuid_datacatalog(self):
        ...
        try:
            ...
            msg["description"] = "%s collection in Biomax for Sample %s" %(collection.get("experiment_type"), collection.get("blSampleId"))
            msg["sampleId"] = collection.get("blSampleId")
        except Exception as ex:
            logging.getLogger("HWR").error("[HWR] Error during data catalog, cannot create initial message: %s" %ex)
        ...
        if self.datacatalog_url:
            try:
                requests.post(self.datacatalog_url, data=json.dumps(msg), headers=headers)
            except Exception as ex:
                ...
                for subitem in collection[item]:
                    if type(subitem) is dict:
                        for subsubitem in subitem:
                            ...
                            smd = self._scientificmetadatawriter(title, subsubitem, subitem[subsubitem])
                            scientificMetadata.update(smd)
                    else:
                        title = "%s: %s" % (item, subitem)
                        smd = self._scientificmetadatawriter(title, subitem, collection[item][subitem])
                        scientificMetadata.update(smd)
