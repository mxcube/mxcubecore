...
class BIOMAXEnergy(AbstractEnergy):
    ...
    def start_move_energy(self, value, wait=True, check_beam=True):
        ...
        if current_en:
            if math.fabs(value - current_en) < 0.001:
                ...
                logging.getLogger("user_level_log").debug("Energy: already at %g, not moving", current_en)
                return
        ...
        if pos < 0.001:
            ...
            if check_beam_end:
                try:
                    ...
                    logging.getLogger("user_level_log").error("Check beam exception: %s" % ex)
    ...
    def check_beam(self):
        ...
        if pidx.State() != PyTango.DevState.MOVING and pidy.State() != PyTango.DevState.MOVING:
        ...
        if self.total_counts < self.min_total_counts:
            ...
            self.total_counts = xbpm.S
            if self.total_counts < self.min_total_counts:
                raise Exception("There is no beam in the BCU. Check the front end shutters, the undulator gap and the NanoBPM regulation.")
        ...
        with gevent.Timeout(120, RuntimeError("The beam is not stable.")):
            while self.is_good_beam(self.N,countsv) == False or self.is_good_beam(self.N,countsh) == False :
                countsv = xbpm.Y
                countsh = xbpm.X
