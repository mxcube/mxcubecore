...
class P11EDNACharacterisation(EDNACharacterisation):
    ...
    def edna_maxwell(self, process_directory, inputxml, outputxml):
        ...
        self.log.debug('=======EDNA========== IN (FUNCTION INPUT) ="%s"' % inputxml)
        self.log.debug('=======EDNA========== OUT (FUNCTION INPUT)="%s"' % outputxml)
        ...
        # srun is not available now. Use script directly.
        # NB!: EDNA output directory is created in the script on the cluster with mkdir -p <dir>
        # to avoid permission problems.
        self.log.debug(
            '=======EDNA Script========== {ssh:s} "{cmd:s}"'.format(ssh=ssh, cmd=cmd)
        )

        os.system('{ssh:s} "{cmd:s}"'.format(ssh=ssh, cmd=cmd))
        ...
        try:
            ...
            self.log.info(f"File '{waitforxml}' is ready for processing.")
