...
class NanoDiff(HardwareObject):
    ...
    def init(self):
        ...
        if self.chan_current_phase is not None:
            ...
            logging.getLogger("HWR").debug('NanoDiff: Current phase channel not defined')
        ...
        try: 
            ...
            if self.image_width is not None and self.image_height is not None:
                ...
                logging.getLogger("HWR").warning('NanoDiff: Zoom center is ' +\
                       'not defined continuing with the middle: %s' % self.zoom_centre)
            else:
                logging.getLogger("HWR").warning('NanoDiff: Neither zoom centre nor camera size iz defined')
    ...
    def move_to_coord(self, x, y, omega=None):
        ...
        if self.current_phase != "BeamLocation":
            try:
                ...
                logging.getLogger("HWR").exception("NanoDiff: could not center to beam, aborting")
        else:
    ...
    def start_centring_method(self, method, sample_info = None):
        ...
        if self.current_centring_method is not None:
            logging.getLogger("HWR").error("NanoDiff: already in centring method %s" % 
        ...
        try:
            ...
            logging.getLogger("HWR").error("NanoDiff: unknown centring method (%s)" % str(diag))
    ...
    def cancel_centring_method(self, reject = False):
        ...
        if self.current_centring_procedure is not None:
            try:
                ...
                logging.getLogger("HWR").exception("NanoDiff: problem aborting the centring method")
            try:
    ...
    def manual_centring(self):
        ...
        # to the beam 
        horizontalCorrection = (self.centring_hwobj.centringDataMatrix[0][0] + self.centring_hwobj.centringDataMatrix[1][0] + self.centring_hwobj.centringDataMatrix[2][0])/3.0 
        ...
        a = numpy.array([[1, math.cos(self.centringPhiValues[0]), math.sin(self.centringPhiValues[0])], \
                         [1, math.cos(self.centringPhiValues[1]), math.sin(self.centringPhiValues[1])], \
                         [1, math.cos(self.centringPhiValues[2]), math.sin(self.centringPhiValues[2])] ])
        
        b = numpy.array([self.centring_hwobj.centringDataMatrix[0][1], self.centring_hwobj.centringDataMatrix[1][1], self.centring_hwobj.centringDataMatrix[2][1]])
    ...
    def emit_centring_successful(self):
        ...
        if self.current_centring_procedure is not None:
            ...
            print("PP___: NanoDiff emit_centring_successful(self) commented code adnout motors  ")
            ...
            logging.getLogger("HWR").debug("NanoDiff: trying to emit centringSuccessful outside of a centring")
    ...
    def snapshots_done(self, snapshots_procedure):
        ...
        try:
            ...
            logging.getLogger("HWR").exception("NanoDiff: could not take crystal snapshots")
    ...
    def visual_align(self, point_1, point_2):
        ...
        """
        if self.in_plate_mode():
            logging.getLogger("HWR").info("NanoDiff: Visual align not available in Plate mode") 
        else:
