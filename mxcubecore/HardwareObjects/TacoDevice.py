...
class TacoDevice(Device):
    ...
    def _init(self):
        ...
        if self.device is None:
            logging.getLogger("HWR").error("%s: could not import Taco Device %s", str(self.name()), self.tacoName())
    ...
    def connectNotify(self, signal):
        ...
        if self.__nb_connections == 1:
            ...
            try:
                ...
                logging.getLogger("HWR").error("%s: cannot start polling, command not set.", self.name())
            else:
                cmdObject.poll(self.interval, self.__polling_args, self.valueChanged, **self.__polling_kwargs)
