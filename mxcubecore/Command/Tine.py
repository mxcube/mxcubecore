...
class TineChannel(ChannelObject):
    ...
    def tineEventCallback(self, id, cc, data_list):
        ...
        elif cc != 103: #  was str(cc) # and self.attributeName not in ("dozor-pass", "ff-ssim"):
    ...
    def get_value(self, force=False):
        ...
        while self.value is None and _counter <= 100 :
           time.sleep(0.020)
           logging.getLogger("HWR").warning('No update after %d ms on: %s %s, executing synchronous get'%(_counter*20,self.tineName,self.attributeName))
