...
class ElementAnalysis(Procedure):
    ...
    MANDATORY_CMDS={"doScan":"doScan","detectorIn":"detectorIn","detectorOut":"detectorOut","setParameters":"setParameters"}
...
#        print "\n\n\n\n\n ElemenyAnalysis:init...................\n\n\n\n"
        ...
        if self.getProperty('specversion') is None:
            logging.getLogger().error('ElementAnalysis: you must specify a spec version')
            self.configOk=False
        else:
            for chan in ElementAnalysis.MANDATORY_CHANNELS:
                ...
                #print "********** - desc: ", desc
                try:
                    channel=self.getChannelObject(chan)    # channel == <SpecChannel> (HardwareRepository.Command.Spec.py)
                    ...
                    channel=None
                if channel is None:
                    logging.getLogger().error("ElementAnalysis: you must specify the %s spec channel"% desc)
                    self.configOk=False
                else:
                    try:
...
#            print "------------commands", commands
            for mandatory_cmd in ElementAnalysis.MANDATORY_CMDS:
                ...
                cmd_found=None
                for cmd in commands:        # cmd == <SpecCommand> (HardwareRepository.Command.Spec.py)
                    #print "--------for cmd in commands: cmd", cmd
                    if cmd.name() == mandatory_cmd:
                        cmd_found=cmd
                        try:
                            exec("cmd.connectSignal('commandReplyArrived', self.%sEnded)"% mandatory_cmd)
                        except AttributeError:
                            pass
                        try:
                            exec("cmd.connectSignal('commandBeginWaitReply', self.%sStarted)"% mandatory_cmd)
                        except AttributeError:
                            pass
                        try:
                            exec("cmd.connectSignal('commandFailed', self.%sFailed)"% mandatory_cmd)
                        except AttributeError:
                            pass
                        try:
                            exec("cmd.connectSignal('commandAborted', self.%sAborted)"% mandatory_cmd)
                        except AttributeError:
                            pass
                        break
                    else:
                ...
                if cmd_found is None:
                    logging.getLogger().error('ElementAnalysis: you must specify the %s command'% desc)
                    self.configOk=False
    ...
    def processElementEdges(self):
        ...
        f.close()
        data=data[4]
        qt.QObject.disconnect(self.xraycalc,qt.SIGNAL("processExited()"),self.processElementEdges)
