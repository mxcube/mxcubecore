...
import asynchat, asyncore, socket, BaseHTTPServer
...
class RequestHandler(asynchat.async_chat, BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def send_head(self):
        ...
        return BaseHTTPServer.BaseHTTPRequestHandler.send_head(self)
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "-r", "--root", dest="root", help="Root path of the web server", action="store"
    )
    parser.add_option(
        "-p",
        "--port",
    ...
    parser.add_option(
        "-0",
        dest="server",
        help="Run the standard server (default)",
        action="store_const",
        const=0,
        default=0,
    )
