...
try:
   ...
   logging.getLogger("HWR").error('Qt4_EMBLMiniDiff: automatic centring not available')
...
class Qt4_EMBLMiniDiff(Equipment):
    """
    Description:
    """	
    ...
    def init(self):
        ...
        self.centring_methods = {
             Qt4_EMBLMiniDiff.MANUAL3CLICK_MODE: self.start_3Click_centring,
             Qt4_EMBLMiniDiff.C3D_MODE: self.start_automatic_centring}
        ...
        self.head_type = HEAD_TYPE.minikappa
        ...
        self.phi_motor_hwobj = self.getDeviceByRole('phi')
        self.phiz_motor_hwobj = self.getDeviceByRole('phiz')
        self.phiy_motor_hwobj = self.getDeviceByRole('phiy')
        self.zoom_motor_hwobj = self.getDeviceByRole('zoom')
        self.focus_motor_hwobj = self.getDeviceByRole('focus')
        self.sample_x_motor_hwobj = self.getDeviceByRole('sampx')
        self.sample_y_motor_hwobj = self.getDeviceByRole('sampy')
       
        if self.head_type == HEAD_TYPE.minikappa:
            self.kappa_motor_hwobj = self.getDeviceByRole('kappa')
            self.kappa_phi_motor_hwobj = self.getDeviceByRole('kappa_phi')
            ...
            if self.kappa_motor_hwobj is not None:
                ...
                logging.getLogger("HWR").error('Qt4_EMBLMiniDiff: kappa motor is not defined')
            ...
            if self.kappa_phi_motor_hwobj is not None:
                ...
                logging.getLogger("HWR").error('Qt4_EMBLMiniDiff: kappa phi motor is not defined')
        else:
        ...
        if self.phiz_motor_hwobj is not None:
            ...
            logging.getLogger("HWR").error('Qt4_EMBLMiniDiff: Phiz motor is not defined')
        ...
        if self.phiy_motor_hwobj is not None:
            ...
            logging.getLogger("HWR").error('Qt4_EMBLMiniDiff: Phiy motor is not defined')
        ...
        if self.zoom_motor_hwobj is not None:
            ...
            logging.getLogger("HWR").error('Qt4_EMBLMiniDiff: Zoom motor is not defined')
        ...
        if self.sample_x_motor_hwobj is not None:
            ...
            logging.getLogger("HWR").error('Qt4_EMBLMiniDiff: Sampx motor is not defined')
        ...
        if self.camera_hwobj is None:
            logging.getLogger("HWR").error('Qt4_EMBLMiniDiff: Camera is not defined')
        else:
        ...
        try: 
            ...
            if self.image_width is not None and self.image_height is not None:
                ...
                logging.getLogger("HWR").warning('Qt4_EMBLMiniDiff: Zoom center is ' +\
                       'not defined continuing with the middle: %s' % self.zoom_centre)
            else:
                logging.getLogger("HWR").warning('Qt4_EMBLMiniDiff: Neither zoom centre nor camera size iz defined')
        ...
        try:
            ...
            logging.getLogger("HWR").warning('Qt4_EMBLMiniDiff: Omega axis is not defined')
        ...
        try:
            ...
            logging.getLogger("HWR").warning('Qt4_EMBLMiniDiff: Grid direction is not defined. Using default.')
    ...
    def in_plate_mode(self):
        self.head_type = self.chan_head_type.getValue()
        return self.head_type == HEAD_TYPE.plate
    ...
    def move_to_coord(self, x, y, omega=None):
        ...
        if self.current_phase != "BeamLocation":
            try:
                ...
                logging.getLogger("HWR").exception("Qt4_EMBLMiniDiff: could not center to beam, aborting")
        else:
    ...
    def start_centring_method(self, method, sample_info = None):
        ...
        if self.current_centring_method is not None:
            logging.getLogger("HWR").error("Qt4_EMBLMiniDiff: already in centring method %s" % 
        ...
        try:
            ...
            logging.getLogger("HWR").error("Qt4_EMBLMiniDiff: unknown centring method (%s)" % str(diag))
            self.emit_centring_failed()
        else:
            try:
                fun(sample_info)
            except:
                logging.getLogger("HWR").exception("Qt4_EMBLMiniDiff: problem while centring")
                self.emit_centring_failed()
    ...
    def cancel_centring_method(self, reject = False):
        ...
        if self.current_centring_procedure is not None:
            try:
                ...
                logging.getLogger("HWR").exception("Qt4_EMBLMiniDiff: problem aborting the centring method")
            try:
    ...
    def emit_centring_successful(self):
        ...
        if self.current_centring_procedure is not None:
            ...
            logging.getLogger("HWR").debug("Qt4_EMBLMiniDiff: trying to emit centringSuccessful outside of a centring")
    ...
    def take_snapshots_procedure(self, image_count, drawing):
        ...
        for index in range(image_count):
            logging.getLogger("HWR").info("Qt4_EMBLMiniDiff: taking snapshot #%d", index + 1)
    ...
    def snapshots_done(self, snapshots_procedure):
        ...
        try:
            ...
            logging.getLogger("HWR").exception("Qt4_EMBLMiniDiff: could not take crystal snapshots")
    ...
    def visual_align(self, point_1, point_2):
        ...
        """
        if self.in_plate_mode():
            logging.getLogger("HWR").info("Qt4_EMBLMiniDiff: Visual align not available in Plate mode") 
        else:
