...
class ExporterZoom(ExporterMotor):
    ...
    def init(self):
        ...
        self.chan_predefined_position = self.addChannel({"type":"exporter",
                                                         "name":"predefined_position"},
                                                         "CoaxialCameraZoomValue")
 
        self.predefined_positions = {"Zoom 1": 1,
                                     "Zoom 2": 2,
                                     "Zoom 3": 3,
                                     "Zoom 4": 4,
                                     "Zoom 5": 5,
    ...
    def sort_predefined_positions_list(self):
        ...
        self.predefined_positions_names_list.sort(\
             lambda x, y: int(round(self.predefined_positions[x] - \
                                    self.predefined_positions[y])))
    ...
    def position_changed(self, position, private={}):
        ...
        if self._last_position_name != position_name:
          ...
          self.emit('predefinedPositionChanged', (position_name, position_name and position or None, ))
    ...
    def zoom_in(self):
        ...
        if position_index < len(self.predefined_positions_names_list) - 1:
            self.move_to_position(self.predefined_positions_names_list[position_index + 1])
