...
class XRFMockup(Equipment):
    ...
    def startXrfSpectrum(self, ct, directory, arch_dir, prefix, session_id=None, blsample_id=None):
        s1 = self.gaussian(numpy.linspace(0, 3, SCAN_LENGTH), 1, 0.1)
        s2 = self.gaussian(numpy.linspace(0, 3, SCAN_LENGTH), 2, 0.3) * (0.25*numpy.random.rand())
        raw_data = s1 + s2
        ...
        with cleanup(self.ready_event.set):
            ...
            for i in xrange(SCAN_LENGTH):
                try:
                    ...
                    self.emit("plot_data", {"id": scan_id,
                                            "data": aux})
                    if divmod(i, SCAN_LENGTH/10)[1] == 0:
                        progress = i / float(SCAN_LENGTH)
                        logging.getLogger('HWR').info('XRF Spectrum Progress %f' %progress)
