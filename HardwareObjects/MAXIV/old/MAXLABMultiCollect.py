...
class MAXLABMultiCollect(AbstractMultiCollect, HardwareObject):
    ...
    def init(self):
        ...
        self.setBeamlineConfiguration(
            synchrotron_name=self.getProperty("synchrotron_name"),
    ...
    def prepare_input_files(
        ...
        self.xds_template = (
            "/home/blissadm/mxCuBE-Qt4/config/dp_file_templates/xdstemplate_maxlab.inp"
        )
    ...
    def write_input_files(self, collection_id):
        ...
        for input_file_dir, file_prefix in (
            ...
            startImageNumber_end = (
                osc_seq["start_image_number"] + osc_seq["number_of_images"] - 5
            )
            ...
            try:
                valdict = {
                    ...
                    # just a hack for now (2013-09-09) should be taken from the lims
                    # system in AbstractMultiCollect.py, so we are certain it's the same
                    "I911-collection_start_time": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        for input_file_dir, file_prefix in (
            ...
            valdict = {
                ...
                "startImageNumber": osc_seq["start_image_number"],
                "axisStart": osc_seq["start"],
                "axisEnd": osc_seq["start"]
                + osc_seq["range"] * osc_seq["number_of_images"]
                - osc_seq["overlap"] * (osc_seq["number_of_images"] - 1),
            }
        ...
        stac_template = (
            "/home/blissadm/mxCuBE-Qt4/config/dp_file_templates/stactemplate_maxlab.inp"
        )
        try:
        ...
        for stac_om_input_file_name, stac_om_dir in (
            ("mosflm.descr", self.mosflm_directory),
        ...
        try:
            ...
            if "inhouse" in directory:
                ...
                archive_dir = os.path.join(
                    "/data/ispyb/", dir_path_list[4], *dir_path_list[5:]
                )
            else:
                archive_dir = os.path.join(
                    "/data/ispyb/", dir_path_list[4], *dir_path_list[5:]
    ...
    def do_collect(self, owner, data_collect_parameters):
                ...
                for start, wedge_size in wedges_to_collect:
                    ...
                    while j > 0:
                        ...
                        with error_cleanup(self.reset_detector):
                            ...
                            # Store image in lims
                            if self.bl_control.lims:
                                if self.store_image_in_lims(
                                    frame, j == wedge_size, j == 1
                                ):
                                    lims_image = {
                            ...
                            if (
                                data_collect_parameters.get("processing", False)
                                == "True"
                            ):
                                self.trigger_auto_processing(
                                    "image",
        ...
        snapshot_filename = "%s_%s_*.snapshot.jpeg" % (
            file_parameters["prefix"],
            file_parameters["run_number"],
        )
        full_snapshot = os.path.join(snapshot_directory, snapshot_filename)
