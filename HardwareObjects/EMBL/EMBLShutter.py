...
class EMBLShutterShutter(Device):
	...
	self.chan_collection_state = self.getChannelObject('chanCollectStatus')
        if self.chan_collection_state is not None:
            self.chan_collection_state.connectSignal('update', self.data_collection_state_changed)

        self.cmd_open_shutter = self.getCommandObject('cmdOpenShutter')
        self.cmd_close_shutter = self.getCommandObject('cmdCloseShutter')

        self.shutter_open_perm_abs_cond = int(self.getProperty('shutterOpenPermissionCondAbs'))
        self.shutter_open_perm_bp_cond = int(self.getProperty('shutterOpenPermissionCondBp')) 
      
        self.chan_shutter_open_perm_abs_value = self.getChannelObject('chanShutterPermCondAbs')
        if self.chan_shutter_open_perm_abs_value is not None:
            self.chan_shutter_open_perm_abs_value.connectSignal('update', self.shutter_perm_abs_value_changed)

        self.chan_shutter_open_perm_bp_value = self.getChannelObject('chanShutterPermCondBp')
        if self.chan_shutter_open_perm_bp_value is not None:
            self.chan_shutter_open_perm_bp_value.connectSignal('update', self.shuuter_perm_bp_value_changed)
        
        self.chan_shutter_is_open = self.getChannelObject('chanShutterIsOpen')
        if self.chan_shutter_is_open is not None:
            self.chan_shutter_is_open.connectSignal('update', self.shutter_is_open_changed)

        self.shutter_is_open_condition = int(self.getProperty("shutterIsOpenCondition"))
    ...
    def open_shutter_thread(self):
        ...
        try:
           ...
           if (not self.is_shuter_open()):
               logging.getLogger().info('Safety shutter: Opening beam shutter a second time is taking some more time....')
               self.cmd_open_shutter("o") 
        except:
