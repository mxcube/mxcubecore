    ...
    def convert_images(self, raw_data_path, flat_field_path):
        ...
        if cut:
            image_arr = dxchange.reader.read_tiff_stack(\
                 ...
                 slc = ((b,d,1),(a,c,1))).astype('float32')
        else:
            image_arr = dxchange.reader.read_tiff_stack(\
                 imlist[0], range(len(imlist))).astype('float32')
        ...
        if cut:
            ...
            ff_arr = dxchange.reader.read_tiff_stack(\
                 fllist[0], range(len(fllist))).astype('float32')
        self.ff_image_arr = ff_arr.transpose(1,2,0)

        num_cores = multiprocessing.cpu_count()
        filtered = Parallel(n_jobs=num_cores)(\
             delayed(self.find_flat)(self.raw_image_arr[:,:,i], self.ff_image_arr) for i in range(self.raw_image_arr.shape[2]))
        self.ff_corrected_image_arr = numpy.transpose(numpy.asarray(filtered), (1,2,0))
    ...
    def do_image_polling(self, exp_time, repeat):
        ...
        while image_index < len(self.display_image_list):
            ...
            if self.repeat_image_play and \
               image_index == len(self.display_image_list):
                image_index = 0
