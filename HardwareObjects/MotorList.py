...
class MotorList(Equipment):
    ...
    def sConnected(self):        
        ...
        speccommand.executeCommand("def _ho_MotorList() '{local md[]; for (i=0; i<MOTORS; i++) {md[motor_mne(i)]=motor_name(i)}; return md}'", wait=True)
        self.addCommand({'name':'MotorListMacro', 'type':'spec', 'version':self.specversion},"_ho_MotorList()")
