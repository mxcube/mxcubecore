...
class Marvin(SampleChanger):
    ...
    def init(self):
        ...
        self.chan_process_step_info = self.getChannelObject(
        ...
        self.chan_command_list = self.getChannelObject("chanCommandList", optional=True)
        ...
        self.chan_sample_is_loaded.connectSignal(
    ...
    def _directlyUpdateSelectedComponent(self, basket_no, sample_no):
        ...
        if basket_no is not None and basket_no > 0 and basket_no <= self._num_basket:
            ...
            if (
                ...
                and sample_no > 0
                and sample_no <= len(basket.getSampleList())
            ):
    ...
    def _doUnload(self, sample_slot=None):
        ...
        if (
            ...
            self.diffractometer_hwobj.set_phase(
                self.diffractometer_hwobj.PHASE_TRANSFER, 60
            )
            if (
                self.diffractometer_hwobj.get_current_phase()
                != self.diffractometer_hwobj.PHASE_TRANSFER
        ...
        if self._focusing_mode == "P13mode":
            self._executeServerTask(self.cmd_unmount_sample, sample_index, basket_index)
        else:
            if self._focusing_mode == "Collimated":
                self._executeServerTask(
