...
class Drop(Container):
    ...
    def __init__(self, cell, well_no):
        super(Drop, self).__init__(
            self.__TYPE__, cell, Drop._getWellAddress(cell, well_no), False
        )
...
class Cell(Container):
    ...
    def __init__(self, container, row, col, wells):
        super(Cell, self).__init__(
            self.__TYPE__, container, Cell._getCellAddress(row, col), False
        )
        self._row = row
        self._col = col
        self._wells = wells
...
class PMSC(SampleChanger):
    ...
    def init(self):
        ...
        self._phase = self.addChannel(
            {"type": self.channel_type, "name": self.channel_phase}, self.channel_phase
        )
        self._taskInfo = self.addChannel(
            {"type": self.channel_type, "name": self.channel_task_info},
            self.channel_task_info,
        )

        self._state.connectSignal("update", self._onStateChanged)
        self._set_phase = self.addCommand(
            {"type": self.channel_type, "name": self.command_set_phase},
            self.command_set_phase,
        )
        self._select_sample = self.addCommand(
            {"type": self.channel_type, "name": self.command_select_sample},
            self.command_select_sample,
        )
        self._abort = self.addCommand(
            {"type": self.channel_type, "name": self.command_abort}, self.command_abort
        )
        self._reset = self.addCommand(
            {"type": self.channel_type, "name": self.command_reset}, self.command_reset
        )
        if self.type == CHANNEL_TANGO:
            self._image = self.addCommand(
                {
                    "type": self.channel_type,
                    "name": "get" + self.channel_image,
                    "timeout": 5,
                },
                "get" + self.channel_image,
            )
        else:
            self._image = self.addChannel(
                {"type": self.channel_type, "name": self.channel_image, "timeout": 5},
                self.channel_image,
            )
