...
class Lakeshore(TacoDevice.TacoDevice):
    def init(self):
        ...
        if self.device.imported:
            ...
            if len(self.ident):
                ...
                logging.getLogger("HWR").error("%s: could not get ident. string from Lakeshore", self.name())
    ...
    def connectNotify(self, signal):
        ...
        if self.__nb_connections == 1:
            logging.getLogger("HWR").debug("%s: starting polling on serial line Device Server %s", self.name(), self.tacoName())
