...
DEFAULT_SCORE_NAME_LIST = ("image_num",
                           "spots_num",
                           "spots_resolution",
                           "score")
...
class GenericParallelProcessing(HardwareObject):
    ...
    def init(self):
        ...
        self.results_name_list = self.getProperty("result_name_list",
                                                  DEFAULT_SCORE_NAME_LIST)
    ...
    def prepare_processing(self):
        ...
        self.params_dict["title"] = "%s_%d_#####.cbf (%d - %d)" % \
             (prefix, run_number, first_image_num, last_image_num)
        self.params_dict["comments"] = "Scan lines: %d, frames per line: %d" % \
             (lines_num, images_num / lines_num)
        self.params_dict["workflow_type"] = \
            self.data_collection.run_processing_parallel
        ...
        self.params_dict["processing_start_time"] = \
        ...
        if lines_num > 1 and grid_params:
            ...
            self.params_dict["reversing_rotation"] = \
                grid_params["reversing_rotation"]
        else:
        ...
        for result_name in self.results_name_list:
            ...
            if self.data_collection.is_mesh():
                self.results_aligned[result_name] = \
                   self.results_aligned[result_name].reshape(
    ...
    def run_processing(self, data_collection):
        ...
        input_filename = os.path.join(self.params_dict["directory"],
                                      "dozor_input.xml")
    ...
    def set_processing_status(self, status):
        ...
        self.data_collection.parallel_processing_result = \
    ...
    def store_processing_results(self, status):
        ...
        if self.params_dict["workflow_type"] in ("MeshScan", "XrayCentering"):
            ...
            try:
                ...
                SimpleHTML.generate_mesh_scan_report(
                    ...
                    html_filename)
                log.info("Parallel processing: Results html saved %s" %
                         html_filename)
            except:
                log.exception("Parallel processing: Could not save results html %s" %
                              html_filename)
        ...
        if self.params_dict["lines_num"] > 1:
            ...
            try:
                ...
                log.exception("Parallel processing: Could not save grid overlay figure %s" % \
                              processing_grid_overlay_file)
            ...
            plt.plot(self.results_aligned["score"] / max_score,
                     ...
                     label="Score",
                     c="r")
            plt.plot(self.results_aligned["spots_num"] / max_spots_num,
                     '.',
            ...
            ax.legend(loc="lower center", fancybox=True, numpoints=1, \
                      borderaxespad=0., bbox_to_anchor=(0.5, -0.13), ncol=3)
            ax.set_ylim(-0.01, 1.1)

            positions = numpy.linspace(0, self.results_aligned["spots_resolution"].max(), 5)
            labels = ['inf']
            ...
            ay1 = ax.twinx()
            new_labels = numpy.linspace(0, self.results_aligned["spots_num"].max(),
                                        len(ay1.get_yticklabels()), dtype=numpy.int16)
        ...
        try:
            ...
            log.info("Parallel processing: Heat map figure %s saved" % \
                processing_plot_file)
        except:
            log.exception("Parallel processing: Could not save figure %s" % \
                processing_plot_file)
        try:
            ...
            log.info("Parallel processing: Archive heat map figure %s saved" % \
                processing_plot_archive_file)
        except:
            log.exception("Parallel processing: Could not save archive figure %s" % \
                processing_plot_archive_file)
    ...
    def align_processing_results(self, start_index, end_index):
        ...
        for score_key in self.results_raw.keys():
            if self.params_dict["lines_num"] > 1:
                ...
                self.results_aligned[score_key] = self.results_raw[score_key][::self.params_dict["images_num"] / self.plot_points_num]
        ...
        if len(index_arr) > 0:
            for index in index_arr:
                if self.results_raw["score"][index] > 0:
                    ...
                    best_position["score"] = \
                        self.results_raw["score"][index]
                    best_position["spots_num"] = \
                        self.results_raw["spots_num"][index]
                    best_position["spots_resolution"] = \
                        self.results_raw["spots_resolution"][index]
