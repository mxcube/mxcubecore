...
class WagoPneu(TacoDevice.TacoDevice):
    ...
    def init(self):
        ...
        if self.device.imported:
            try:
                ...
                if self.readmode == WagoPneu.READ_OUT:
                    # (%s)[0]' % self.wagokyout)
                    self.setPollCommand("DevReadNoCacheDigi", self.wagokyout)
                elif self.readmode == WagoPneu.READ_CMD:
                    # (%s)[0]' % self.wagokyin)
                    self.setPollCommand("DevReadNoCacheDigi", self.wagokyin)
    ...
    def wagoIn(self):
        if self.isReady():
            ...
            self.device.DevWriteDigi([self.wagokyin, 0, 1])
    ...
    def wagoOut(self):
        if self.isReady():
            ...
            # executeCommand('DevWriteDigi(%s)' % str(self.argin))
            self.device.DevWriteDigi([self.wagokyin, 0, 0])
