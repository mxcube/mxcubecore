        ...
        if self.device.imported:
             try:
                ...
                if self.readmode == WagoPneu.READ_OUT:
                 ...
                 logging.getLogger("HWR").exception("%s: cannot find id for wago", self.name())
             else:
                if self.readmode == WagoPneu.READ_OUT:
                    self.setPollCommand('DevReadNoCacheDigi', self.wagokyout) #(%s)[0]' % self.wagokyout)
                elif self.readmode == WagoPneu.READ_CMD:
                    self.setPollCommand('DevReadNoCacheDigi', self.wagokyin) #(%s)[0]' % self.wagokyin)
