...
def prepare(centring_motors_dict):
    ...
    SAVED_INITIAL_POSITIONS = dict(
        [(m.motor, m.motor.getPosition()) for m in centring_motors_dict.itervalues()]
    )
...
def centre_plate1Click(
    ...
    try:
        ...
        # it is now a while true loop that can be interrupted at any time by the
        # save button, to allow user to have a 1 click centring as precise as he
        # wants (see HutchMenuBrick)
        while True:
            USER_CLICKED_EVENT = gevent.event.AsyncResult()
            try:
            ...
            # centring point
            if i % 2 == 0:
                # in case the phi range sent us to a position where sample is invisible,
                # if user moves phi, this modifications is saved for future moves
                phi_min = phi.getPosition()
                phi.move(phi_max)
            else:
                # in case the phi range sent us to a position where sample is invisible,
                # if user moves phi, this modifications is saved for future moves
                phi_max = phi.getPosition()
                phi.move(phi_min)
