...
The JSON schemas that give rise so the images shown here can be found in the mxcubecore repository, github develop branch, in `mxcubecore/docs/source/assets/json_schema_gui`.
...
Another example of a pre-acquisition popup. Note that in this image row 12 is selected. The update information generated (successively) by selecting this row, by setting Crystal Lattice to 'Cubic', and by setting Space Group to F432 can be found in [update_indexing.json](/assets/json_schema_gui/update_indexing.json), [update_lattice.json](/assets/json_schema_gui/update_lattice.json), and [update_spacegroup.json](/assets/json_schema_gui/update_spacegroup.json), respectively (in `mxcubecore/docs/source/assets/json_schema_gui`)
...
The dose and dose budget fields are coloured yellow as a warning because after increasing the transmission value, the dose value is higher than the dose budget. Editing the transmission field has also coloured that filed orange, as 'most recently edited'. Note that the Continue button is still active. This is done through update functions. The update information generated by changing the transmission can be found in [update_transmission.json](/assets/json_schema_gui/update_transmission.json) (in `mxcubecore/docs/source/assets/json_schema_gui`)
